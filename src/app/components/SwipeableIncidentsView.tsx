...
const SwipeableIncidentsView = ({
  onFiltersOpen,
  onIncidentNavigate,
}: SwipeableIncidentsViewProps) => {
  ...
  // State variables to control the full expansion of the panel
  // isFullyExpanded: true when the panel is at full height
  // touchCount: counts touches to enable full expansion on the second touch
  ...
  const handleFiltersChange = useCallback(
    (newFilters: IncidentFiltersType) => {
      if (!isEditorOrAdmin) {
  ...
  const handleIncidentSelect = useCallback(
    (incident: Incident) => {
      // Actualizar el incidente seleccionado y colapsar la lista
      setSelectedIncident(incident)
      setBottomSheetHeight(20)
      setIsExpanded(false)
  ...
  const handleCloseModal = useCallback((e: React.MouseEvent) => {
    ...
    // Reset related states to ensure clean modal close
    setIsExpanded(true)
    setBottomSheetHeight(40)
    // Force a reflow to ensure the overlay is removed
    document.body.style.overflow = ''
  }, [])
  ...
  const handleTouchStart = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (
        isExpanded ||
        (e.target as HTMLElement).closest('[data-bottom-sheet]')
      ) {
        return
      }
  ...
  const handleBottomSheetDrag = useCallback(
  ...
  return (
    <>
        ...
        {selectedIncident && (
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center p-4">
              <div
                className="bg-[#0f1317] rounded-2xl w-full max-w-[720px] max-h-[95vh] overflow-hidden border border-white/10 shadow-[0_24px_80px_rgba(0,0,0,0.65),0_2px_8px_rgba(0,0,0,0.4)] animate-[modalIn_220ms_ease-out]"
                role="document"
              >
                ...
                <div className="p-5 overflow-y-auto max-h-[calc(90vh-112px)]">
                  <IncidentDetails
      ...
      >
        ...
        <motion.div
          data-bottom-sheet
          className={`fixed left-0 right-0 md:hidden backdrop-blur-lg rounded-t-2xl shadow-2xl ${
            isExpanded ? 'z-[200]' : 'z-[150]'
          }`}
          drag="y"
          ...
          style={{
            background:
              'linear-gradient(180deg, rgba(25, 25, 25, 1) 0%, rgba(20, 20, 20, 1) 100%)',
        ...
        >
          ...
          <div className="px-4 pb-3 border-b border-gray-700/30 cursor-pointer hover:bg-gray-800/20 transition-all duration-300">
                  ...
                  return (
                    <div key={incident._id}>
                      <motion.div
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          // Set the selected incident and collapse the view
                          setSelectedIncident(incident);
                        }}
                      >
                            <div className="flex items-start space-x-4">
                              ...
                              {getIncidentImage(incident) ? (
                                ...
                                <motion.div
                                  className={`p-3 rounded-xl ${getIncidentTypeColor(
                                    incident.type
                                  )} text-white shadow-lg relative overflow-hidden flex-shrink-0`}
                                  whileHover={{ scale: 1.05 }}
                                  transition={{ duration: 0.2 }}
                                >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-start justify-between mb-3">
                                  <h4 className="font-manrope font-semibold text-white text-base truncate pr-2">
                                    {incident.tags?.[0]}
                                  </h4>
                                  <span className="text-xs font-medium px-2.5 py-1.5 rounded-lg flex-shrink-0 bg-gray-600/50 text-gray-300 border border-gray-500/30">
                                ...
                                <div className="pt-3 border-t border-gray-600/30">
                                  <motion.button
                                    className="mt-2 w-full py-2 px-3 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/30 rounded-lg text-blue-300 text-sm font-medium transition-colors flex items-center justify-center gap-2"
                                    whileHover={{ scale: 1.02 }}
                                    whileTap={{ scale: 0.98 }}
                                  >
                                    <svg
