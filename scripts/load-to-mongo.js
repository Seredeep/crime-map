...
// Ruta al archivo GeoJSON corregido
const FIXED_FILE = path.join(__dirname, 'barrios-fixed.geojson');
...
async function loadGeoJSONToMongoDB(filePath) {
  ...
  try {
    ...
    console.log('Leyendo archivo GeoJSON corregido...');
    const fileContent = fs.readFileSync(filePath, 'utf8');
    
    if (!fileContent || fileContent.trim() === '') {
      throw new Error('El archivo GeoJSON está vacío');
    }
    
    const geoJSONData = JSON.parse(fileContent);
    
    if (!geoJSONData.features || !Array.isArray(geoJSONData.features)) {
      throw new Error('Formato de GeoJSON inválido: no se encontraron features');
    }
    
    console.log(`Archivo cargado con ${geoJSONData.features.length} features`);
...
async function main() {
  try {
    console.log(`Usando archivo GeoJSON corregido: ${FIXED_FILE}`);
    const insertedCount = await loadGeoJSONToMongoDB(FIXED_FILE);
